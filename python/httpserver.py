...
class ToDo:
    ...
    def showTips(self):
        ...
        try:
            ...
            print ToDo().getTimeNow() + 'You have not give a port, plase use Chrome Browser:http://' + ToDo().getip() + ':' + str(
                port)
    ...
    def do_utf82gbk(self, sts):
        if ("Windows" in platform.system()):
            return sts.decode('utf-8').encode('gbk')
        else:
            return sts

    def do_gbk2utf8(self, sts):
        if ("Windows" in platform.system()):
            return sts.decode('gbk').encode('utf-8')
        else:
            return sts
...
class SimpleHTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def returnMessage(self, r, info):
        ...
        f.write('<input type=\"button\" value=\"返回\" onClick=\"javascript:history.back(-1);\">\n')
        f.write(
    ...
    def list_directory(self, path):
        ...
        #f.write('<table><tr><td>')
        #f.write("<input type=\"button\" value=\"返回\" onClick=\"javascript:history.back(-1);\"></td></tr><tr><td>")
        f.write('<table><tr><td>')
        f.write("<form ENCTYPE=\"multipart/form-data\" method=\"post\">")
        f.write("<input name=\"file\" type=\"file\"/>")
        f.write("<input type=\"submit\" value=\"上传\"/>")
        f.write("<input type=\"button\" value=\"返回\" onClick=\"javascript:history.back(-1);\">")
        f.write("</form></td></tr><tr><td>")
        ...
        for name in list:
            ...
            emd5 = "<a id=a_%d οnclick=callBackMD5('?getMD5=%s','a_%d')>MD5</a>" % (idn, urllib.quote(linkname), idn)
